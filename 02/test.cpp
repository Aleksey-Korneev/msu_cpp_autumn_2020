...
void Test2()
{
    ...
    std::vector<std::string> tokens = { "Text", "123", "ExtraText", "2", "af", "x3vb", "009a" };
    assert(parser.GetTokens() == tokens);
}
...
void Test4()
{
    ...
    parser.SetEndCallback([&time_start]{
            ...
            << " seconds" << std::endl; });
    parser.Parse();
    std::vector<std::string> tokens = { "This", "is", "an", "original", "\\ttext", "!" };
    assert(parser.GetTokens() == tokens);
...
void RunTests()
{
    const std::vector<std::function<void()>> tests = { Test1, Test2, Test3, Test4 };
